...
class DialogSettings(QDialog):
    def __init__(self, parent=None):
        ...
        self.original_theme = self.config_data.get(
            "THEME", Constants.SETTINGS_THEME_DARK
        )
    ...
    def create_general_tab(self):
        ...
        self.theme_combo.addItems(
            [
                Constants.SETTINGS_THEME_LIGHT,
                Constants.SETTINGS_THEME_DARK,
                Constants.SETTINGS_THEME_CLASSIC,
            ]
        )
        ...
        self.latest_days_spin.setValue(
            int(self.config_data.get("LATEST_DURATION_DAYS", 7))
        )
        ...
        self.initial_index_combo.addItems(
            [Constants.SETTINGS_INITIAL_BEGINNING, Constants.SETTINGS_INITIAL_END]
        )
        initial_index = self.config_data.get(
            "INITIAL_MEDIA_INDEX", Constants.SETTINGS_INITIAL_BEGINNING
        )
        self.initial_index_combo.setCurrentText(initial_index)
        general_layout.addRow(
            Constants.SETTINGS_INITIAL_INDEX, self.initial_index_combo
        )

        # Delete original after upload
        self.delete_original_check = QCheckBox()
        self.delete_original_check.setChecked(
            self.config_data.get("DELETE_ORIGINAL_AFTER_UPLOAD", False)
        )
        general_layout.addRow(
            Constants.SETTINGS_DELETE_ORIGINAL, self.delete_original_check
        )
    ...
    def create_storage_tab(self):
        ...
        self.local_storage_check.setChecked(
            self.config_data.get("LOCAL_STORAGE_ENABLED", True)
        )
        storage_layout.addRow(
            Constants.SETTINGS_LOCAL_STORAGE, self.local_storage_check
        )
        ...
        storage_layout.addRow(
            Constants.SETTINGS_THUMBNAILS_DIR, self.thumbnails_dir_edit
        )
    ...
    def create_cloud_tab(self):
        ...
        # CloudFront settings
        self.cloudfront_domain_edit = QLineEdit(
            self.config_data.get("CLOUDFRONT_DOMAIN", "")
        )
        cloud_layout.addRow(
            Constants.SETTINGS_CLOUDFRONT_DOMAIN, self.cloudfront_domain_edit
        )

        self.cloudfront_key_id_edit = QLineEdit(
            self.config_data.get("CLOUDFRONT_KEY_ID", "")
        )
        cloud_layout.addRow(
            Constants.SETTINGS_CLOUDFRONT_KEY_ID, self.cloudfront_key_id_edit
        )

        self.cloudfront_key_path_edit = QLineEdit(
            self.config_data.get("CLOUDFRONT_KEY_PATH", "")
        )
        cloud_layout.addRow(
            Constants.SETTINGS_CLOUDFRONT_KEY_PATH, self.cloudfront_key_path_edit
        )
    ...
    def on_theme_changed(self, new_theme):
        ...
        if new_theme != self.original_theme:
            log(
                "DialogSettings.on_theme_changed",
                f"Theme will change to: {new_theme}",
                level="info",
            )
            show_message(
    ...
    def save_settings(self):
        ...
        if Config.update_settings(settings_to_update):
            ...
            log(
                "DialogSettings.save_settings", "Failed to save settings", level="error"
            )

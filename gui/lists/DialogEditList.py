...
class DialogEditList(DialogReorder):
    def __init__(self, media_loader, media_list_manager, list_name, list_names, parent=None):
        self.media_loader = media_loader
        self.media_list_manager = media_list_manager
        self.list_name = list_name
        self.list_names = list_names

        self.thumbnail_keys = [f"{uuid}.jpg" for uuid in self.media_list_manager.get_uuids_from_list(self.list_name)]
        
        super().__init__(thumbnail_keys=self.thumbnail_keys, media_loader=self.media_loader)
    ...
    def on_ok_button_clicked(self):
        ...
        if new_name == self.list_name or new_name not in self.list_names:
            ...
            show_message("Aynı adda bir liste zaten var. Başka bir ad girin.", level="warning")
